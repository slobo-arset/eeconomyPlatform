...
@Component({
  selector: 'app-budzetski-rokovi',
  templateUrl: './budzetski-rokovi.component.html',
  styleUrl: './budzetski-rokovi.component.scss'
})
export class BudzetskiRokoviComponent {
  firstDate: Date;
  items: MenuItem[] = [{ label: 'e-Rokovi (Budžetski rokovi)' }];
  ...
  ngOnInit(): void {
    ...
    this.info$ = this.infoService.getAllErokovi(this.selectedMonth, 1, this.params)
  ...
  create(){
    ...
    this.info$ =  this.infoService.createErokovi(data).pipe(
      switchMap(()=>{
        return this.infoService.getAllErokovi(this.selectedMonth, 1, this.params);
      }),
      tap(()=>{
       this.visible = false;
      })
